...
remove_UnicodeEmojis <- function(x){
  y <- gsub("[^\x01-\x7F]", "", x)
  return(y)
}
...
cleanCorpus<-function(corpus, customStopwords){
  ...
  corpus <- tm_map(corpus, content_transformer(remove_UnicodeEmojis))
...
cleanCorp <- function(pth, columnName, collapse = F, customStopwords){
...
cleanMatrix <- function(txtCorpus, type, wgt){
  if(type =='TDM'){
    if(wgt == 'weightTfIdf'){
